...
def _route_after_execute(state: JobAgentState) -> str:
    result = state.get("apply_result", {})
    if state.get("errors"):
        return "error"
    return "save" if result.get("success") else "error"


def build_remotive_graph():
    g = StateGraph(JobAgentState)

    g.add_node("collect_preferences", collect_preferences.run)
    g.add_node("parse_resume", parse_resume.run)
    g.add_node("fetch_jobs", fetch_jobs.run)
    g.add_node("rank_jobs", rank_jobs.run)
    g.add_node("select_next_job", select_next_job.run)
    g.add_node("customize_resume", customize_resume.run)
    g.add_node("generate_cover_letter", generate_cover_letter.run)
    g.add_node("capture_job_page", capture_job_page.run)
    g.add_node("plan_application", plan_application.run)
    g.add_node("execute_application", execute_application.run)
    g.add_node("save_application", save_application.run)
    g.add_node("error_handler", error_handler.run)
    ...
    g.add_edge("select_next_job", "customize_resume")
    g.add_edge("customize_resume", "generate_cover_letter")
    g.add_conditional_edges("generate_cover_letter", _has_quota_error, {"ok": "capture_job_page", "error": END})
    g.add_edge("capture_job_page", "plan_application")
    g.add_conditional_edges("plan_application", _has_quota_error, {"ok": "execute_application", "error": END})
    g.add_conditional_edges("execute_application", _route_after_execute, {
        "save": "save_application",
        "error": "error_handler",
    })
